...
export function MemberDetail({ id, apiBase, backHref, label }: MemberDetailProps) {
  ...
  if (!member) return <p className="text-muted-foreground">{label} not found</p>;

  const totalValue = member.quotations.reduce((s, q) => s + q.grandTotal, 0);
    ...
    <div className="space-y-6 max-w-4xl">
      <div className="flex items-center gap-3">
        ...
        <div>
          ...
          <p className="text-sm text-muted-foreground mt-0.5">{label} details and history</p>
        </div>
      </div>
          ...
          <CardContent>
            <div className="grid grid-cols-2 gap-4">
              <div>
                ...
                <p className="text-2xl font-bold">{member.quotations.length}</p>
              </div>
              <div>
